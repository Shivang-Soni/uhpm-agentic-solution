...
async def test_full_uhpm_graph_integration():
    ...
    with patch(
        "backend.graph.uhpm_graph.planner_agent.plan",
        return_value=fake_plan), \
        patch(
             "backend.graph.uhpm_graph.reasoner.decide",
             return_value=fake_reason), \
        patch(
            "backend.graph.uhpm_graph.research_agent.analyse_product",
            return_value=fake_research), \
        patch(
            "backend.graph.uhpm_graph.persona_agent.generate_persona",
            return_value=fake_persona), \
        patch(
            "backend.graph.uhpm_graph.content_agent.generate_content",
            return_value=fake_content), \
        patch(
            "backend.graph.uhpm_graph.experiment_agent.score_variants",
            return_value=fake_experiment), \
        patch(
            "backend.graph.uhpm_graph.analytics_agent.analyse_campaign",
